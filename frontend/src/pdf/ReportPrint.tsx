...
export function ReportPrint() {
  ...
  const kpis: { label: string; value: string; accent?: string }[] = [
    ...
    { label: t('kpi.cac'), value: formatRM(ch.blendedCAC, lang, ch.blendedCAC > 0 && ch.blendedCAC < 1 ? 2 : 0) },
    { label: t('traffic.paidCac'), value: formatRM(ch.paidCAC, lang, ch.paidCAC > 0 && ch.paidCAC < 1 ? 2 : 0), accent: PILLAR_COLOR.traffic },
    { label: t('kpi.ltv'), value: formatRM(rev.ltv, lang) },
    { label: t('kpi.ltvCac'), value: rev.cac > 0 ? formatMultiplier(rev.ltvToCac, 1) : '—' },
